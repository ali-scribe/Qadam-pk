...
import { MOCK_ACTION_PLAN, MOCK_DOCUMENT_ANALYSIS } from "@/lib/mockData";
import type { EligibilityVerdict } from "@/lib/types";

export const metadata = {
  title: "Your Action Plan — Qadam",
};
...
export default function ResultPage() {
  const plan = MOCK_ACTION_PLAN;
  const doc = MOCK_DOCUMENT_ANALYSIS;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-4xl px-4 py-10 space-y-8">
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 flex items-start gap-2">
          ...
          <span>
            <strong>Demo mode:</strong> This result uses example data for an
            HEC Need-Based Scholarship. Upload a real document on the{" "}
            <Link href="/analyze" className="underline hover:text-amber-900">
              Analyze page
            </Link>{" "}
            to get a personalised plan.
          </span>
        </div>
        ...
        <div className="rounded-2xl bg-white px-6 py-5 shadow-sm ring-1 ring-gray-100">
          ...
          <p className="mt-1 text-sm text-gray-600">
            {doc.purpose ?? "Purpose not stated in document"}
          </p>
          {doc.unknownFields.length > 0 && (
            <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
              <p className="text-xs font-medium text-gray-500">
                Could not determine from document
              </p>
              <ul className="mt-1 list-disc list-inside space-y-0.5">
                {doc.unknownFields.map((f) => (
        ...
        {/* ── Eligibility verdict ── */}
        <div
          className={`rounded-2xl border-l-4 px-6 py-5 shadow-sm ${verdict.bg} ${verdict.border}`}
        >
          <div className="flex items-center gap-3">
            <span
            ...
            <h2 className={`text-xl font-bold ${verdict.text}`}>
              {verdict.label}
            </h2>
          </div>
          ...
          <p className="mt-4 text-xs text-gray-500 italic">
            Qadam is not a legal advisor. Always verify your eligibility
            directly with the issuing authority before submitting.
          </p>
        </div>
        ...
        <section aria-labelledby="criteria-heading">
          ...
          <div className="space-y-4">
            ...
            <CriteriaSection
              ...
              items={
                plan.satisfiedCriteria.length > 0
                  ? plan.satisfiedCriteria.map((c) => ({
                      text: c.text,
                      sub: null,
                      evidence: c.evidence,
                    }))
            ...
            <CriteriaSection
              ...
              items={
                plan.unmetCriteria.length > 0
                  ? plan.unmetCriteria.map((c) => ({
                      text: c.text,
                      sub: c.reason,
                      evidence: c.evidence,
                    }))
            ...
            <CriteriaSection
              ...
              items={
                plan.unknownCriteria.length > 0
                  ? plan.unknownCriteria.map((c) => ({
                      text: c.text,
        ...
        <section aria-labelledby="deadlines-heading">
          ...
          <div className="rounded-2xl bg-white shadow-sm ring-1 ring-gray-100 overflow-hidden">
            {doc.deadlines.length === 0 ? (
              <p className="px-5 py-4 text-sm text-gray-500">
                Deadline: Not stated in document
              </p>
            ) : (
              <ul className="divide-y divide-gray-100">
        ...
        <div className="flex justify-center pt-4 pb-10">
          <Link
            href="/analyze"
            className="rounded-lg border border-gray-300 bg-white px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors min-h-[44px] flex items-center"
          >
...
// ─── CriteriaSection helper component ────────────────────────────────────────
...
interface CriteriaSectionProps {
  ...
  icon: string;
  iconColor: string;
  items: CriteriaItem[] | null; // null = empty
}
          ...
          {items.map((item, i) => (
            <li key={i} className="px-4 py-3">
              <div className="flex items-start gap-2">
                <span
                  className={`mt-0.5 flex-shrink-0 text-sm font-bold ${iconColor}`}
                  aria-hidden="true"
                >
                  {icon}
                </span>
